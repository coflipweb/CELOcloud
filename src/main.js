...
const MPContractAddress = "0xEBDBadC554BD8AB133265F88580acC2A9f0AE2d0"
...
if (!hash || !name || !description || !location) {
  ...
  await contract.methods.uploadFile(hash, name, description, location)
...
const files = await contract.methods.getFiles(uploadedFiles).call();
...
files.forEach(file => {
  ...
  li.innerHTML = `
    ...
    <div>File location: ${file.location}</div>
  `;
